        ...
        <CardContent sx={{ p: { xs: 1.5, md: 2.25 } }}>
          ...
          {slaFlow && <Card variant="outlined" sx={{mb:2}}>
            <CardContent>
              ...
              <Typography sx={{fontWeight:850,mt:2,mb:.25}}>Tempo por Bug e Task</Typography><Typography variant="body2" color="text.secondary" sx={{mb:1}}>Os números à esquerda são os <strong>IDs dos atendimentos Movidesk</strong>. Cada linha corresponde a um Bug vinculado a uma Task Azure. As barras comparam o tempo útil de Suporte e Fábrica; passe o mouse para ver a Task e clique na barra para abrir o detalhamento.</Typography><Box sx={{mt:1,height:Math.max(220,Math.min(420,(slaFlow.rows?.length??0)*30))}}>
                <ResponsiveContainer width="100%" height="100%"><BarChart data={(slaFlow.rows??[]).slice(0,12)} layout="vertical" margin={{left:10,right:18}}><CartesianGrid stroke={theme.palette.divider} strokeDasharray="3 3" opacity={.35}/><XAxis type="number" tick={{fill:theme.palette.text.secondary}} tickFormatter={(v)=>`${Math.round(v/60)}h`}/><YAxis type="category" dataKey="movideskId" width={78} tick={{fill:theme.palette.text.secondary,fontWeight:700}}/><ChartTooltip contentStyle={{backgroundColor:theme.palette.background.paper,border:`1px solid ${theme.palette.divider}`,borderRadius:10,color:theme.palette.text.primary}} labelStyle={{color:theme.palette.text.primary,fontWeight:800}} formatter={(v:any,n:any,p:any)=>[formatMinutes(Number(v)),`${n==="supportMinutes"?"Suporte":"Fábrica"} · Task #${p?.payload?.taskNumber??"—"}`]}/><Bar dataKey="supportMinutes" name="Suporte" fill={aliareColors.info} radius={[0,5,5,0]} maxBarSize={16} cursor="pointer" onClick={(r:any)=>setSlaDrilldown({title:`Atendimento #${r.movideskId} · Task #${r.taskNumber}`,ids:[r.movideskId]})}/><Bar dataKey="factoryMinutes" name="Fábrica" fill={aliareColors.green} radius={[0,5,5,0]} maxBarSize={16} cursor="pointer" onClick={(r:any)=>setSlaDrilldown({title:`Atendimento #${r.movideskId} · Task #${r.taskNumber}`,ids:[r.movideskId]})}/></BarChart></ResponsiveContainer>
              </Box>
