...
export function MyOperation() {
  ...
  return <Box sx={{ pt: 5, pb: 4, minHeight: 0 }}>
    ...
    <Drawer anchor="right" open={Boolean(selected)} onClose={() => { setSelected(null); setDetail(null); }} slotProps={{ paper: { sx: { width: { xs: "100%", sm: 620 }, p: 3 } } }}>
      ...
      <Stack spacing={1.2} sx={{ mt: 2 }}>{selected && Object.entries({ Estado: selected.status, "Cliente principal": detail?.ticket.client ?? selected.client, "Clientes participantes": formatList(selected.workItem?.participantClients), Contato: detail?.ticket.contact, Categoria: detail?.ticket.category, Urgência: detail?.ticket.urgency, Serviço: detail?.ticket.service, Equipe: detail?.ticket.ownerTeam, Responsável: detail?.ticket.owner ?? selected.workItem?.assignedToName, "Prazo de solução": detail?.ticket.dueDate, Versão: selected.workItem?.deliveredVersion, "Ticket principal": selected.workItem?.movideskTicket, "Tickets participantes": formatList(selected.workItem?.participantMovideskTickets), "Task relacionada": selected.ticket?.taskNumber }).map(([label, value]) => <Box key={label}><Typography variant="caption" color="text.secondary">{label}</Typography><Typography>{String(value ?? "Não informado")}</Typography></Box>)}</Stack>
