...
type Gap = { id: string; type: string; title: string; evidence: string; impact: string; action: string; status: string; confidence?: string; ticketCount?: number; azureLinked?: number; blockedLinked?: number; deliveredLinked?: number; examples?: Ticket[] };
    ...
    <Card sx={{
      ...
      <CardContent sx={{ p: { xs: 2, md: 2.5 } }}>
        ...
        {data && <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr 1fr", md: "repeat(4,1fr)" }, gap: 1.2, mt: 2 }}>
          ...
          ].map(([label, value]) => <Box key={String(label)} sx={{ p: 1.4, borderRadius: 2, border: "1px solid", borderColor: mode === "dark" ? "rgba(255,255,255,.09)" : "rgba(15,23,42,.08)", background: mode === "dark" ? "linear-gradient(145deg, rgba(19,69,91,.62), rgba(18,35,62,.58))" : "rgba(255,255,255,.72)", boxShadow: mode === "dark" ? "inset 0 1px rgba(255,255,255,.035)" : "0 5px 18px rgba(15,23,42,.035)" }}><Typography variant="caption" sx={{ color: mode === "dark" ? "rgba(255,255,255,.55)" : "text.secondary" }}>{label}</Typography><Typography sx={{ fontWeight: 900, fontSize: "1.45rem", mt: .25 }}>{value}</Typography></Box>)}
        </Box>}
      </CardContent>
    </Card>
