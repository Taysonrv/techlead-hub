      ...
      <Box sx={{ display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <Box ref={messagesRef} sx={{ flex: 1, minHeight: 0, overflowY: "auto", overscrollBehavior: "contain", p: { xs: 1.5, md: 2.25 }, bgcolor: (theme) => theme.palette.mode === "dark" ? "rgba(255,255,255,.012)" : "#f8fafb", backgroundImage: (theme) => theme.palette.mode === "dark" ? "radial-gradient(circle at 50% 0%, rgba(24,199,122,.035), transparent 36%)" : "radial-gradient(circle at 50% 0%, rgba(24,199,122,.045), transparent 38%)" }}>
          {loading ? <Box sx={{ display: "grid", placeItems: "center", minHeight: 160 }}><CircularProgress size={28} /></Box> : visibleMessages.map((message, messageIndex) => {
            ...
            return <Box key={message.id} sx={{ display: "flex", justifyContent: mine ? "flex-end" : "flex-start", mb: sameAuthor ? .25 : 1.1, mt: sameAuthor ? 0 : .6 }}><Paper elevation={0} sx={{ maxWidth: { xs: "88%", md: "68%" }, p: "9px 12px", bgcolor: mine ? "primary.main" : mentioned ? "rgba(245,158,11,.10)" : "background.paper", color: mine ? "primary.contrastText" : "text.primary", border: mine ? 0 : "1px solid", borderColor: mentioned ? "warning.main" : "divider", borderRadius: mine ? "16px 16px 4px 16px" : "16px 16px 16px 4px", boxShadow: mine ? "0 8px 20px rgba(24,199,122,.12)" : "0 5px 16px rgba(15,23,42,.045)" }}><Stack direction="row" spacing={1} sx={{ justifyContent: "space-between", alignItems: "center" }}><Typography variant="caption" sx={{ fontWeight: 800, opacity: .8 }}>{sameAuthor ? "" : message.author.name}</Typography><Tooltip title="Responder"><IconButton size="small" onClick={() => setReplyTo(message)} sx={{ color: "inherit", opacity: .65 }}><ReplyOutlined sx={{ fontSize: 16 }} /></IconButton></Tooltip></Stack>{parent && <Box sx={{ px: 1, py: .6, mb: .6, borderLeft: "3px solid", borderColor: mine ? "rgba(255,255,255,.55)" : "primary.main", bgcolor: mine ? "rgba(255,255,255,.12)" : "action.hover", borderRadius: 1 }}><Typography variant="caption" sx={{ fontWeight: 800 }}>{parent.author.name}</Typography><Typography variant="caption" noWrap sx={{ display: "block", maxWidth: 420 }}>{parent.content}</Typography></Box>}{attachmentInfo(message.content) ? (() => { const attachment = attachmentInfo(message.content)!; return <Paper component="a" href={attachment.href} download={attachment.name} elevation={0} sx={{ display: "flex", alignItems: "center", gap: 1, p: 1, minWidth: 210, maxWidth: 340, textDecoration: "none", color: "inherit", bgcolor: mine ? "rgba(255,255,255,.16)" : "background.paper", border: "1px solid", borderColor: mine ? "rgba(255,255,255,.22)" : "divider", borderRadius: 2 }}><Box sx={{ width: 34, height: 34, borderRadius: 1.5, display: "grid", placeItems: "center", bgcolor: "action.hover" }}><AttachFileRounded fontSize="small" /></Box><Box sx={{ minWidth: 0, flex: 1 }}><Typography variant="body2" sx={{ fontWeight: 800 }} noWrap>{attachment.name}</Typography><Typography variant="caption" color="text.secondary">{Math.max(1, Math.round(attachment.size / 1024))} KB</Typography></Box><DownloadRounded fontSize="small" /></Paper>; })() : <Typography sx={{ whiteSpace: "pre-wrap", overflowWrap: "anywhere" }}>{renderContent(message.content)}</Typography>}<Typography variant="caption" sx={{ display: "block", textAlign: "right", opacity: .7 }}>{new Date(message.createdAt).toLocaleString("pt-BR")}</Typography></Paper></Box>;
          })}
          <div ref={bottomRef} />
        </Box>
        <Divider />
        ...
        <Stack direction="row" spacing={.7} sx={{ p: 1, alignItems: "center", bgcolor: "background.paper" }}>
