...
function Branch({node,level=0,expanded,active,onToggle,onSelect}:{node:ViewNode;level?:number;expanded:Set<number>;active:number|null;onToggle:(id:number)=>void;onSelect:(n:SimerTreeNode)=>void}){
 ...
 return <Box id={`simer-node-${node.id}`} sx={{position:"relative",pl:level?2.4:0,scrollMargin:80}}>
  ...
  {open&&children&&<Box sx={{ml:1.5,borderLeft:"1px solid",borderColor:"divider"}}>{node.children.map(child=><Branch key={child.id} node={child} level={level+1} expanded={expanded} active={active} onToggle={onToggle} onSelect={onSelect}/>)}</Box>}
 </Box>;
}
export function SimerMapTree({items,focusId,onSelect}:{items:SimerTreeNode[];focusId?:number|null;onSelect:(n:SimerTreeNode)=>void}){
 ...
 return <Box sx={{height:"68vh",minHeight:480,overflow:"auto",p:2,border:"1px solid",borderColor:"divider",borderRadius:3,bgcolor:"background.default"}}>
  ...
  <Box sx={{minWidth:"max-content",pb:3}}>{roots.map(root=><Branch key={root.id} node={root} expanded={expanded} active={active} onToggle={id=>setExpanded(v=>{const n=new Set(v);n.has(id)?n.delete(id):n.add(id);return n;})} onSelect={node=>{setActive(node.id);onSelect(node);}}/>)}</Box>
 </Box>;
}