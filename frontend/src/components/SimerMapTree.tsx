...
const Branch=memo(function Branch({node,level=0,expanded,active,onToggle,onSelect,onFollowLink}:{node:ViewNode;level?:number;expanded:Set<number>;active:number|null;onToggle:(id:number)=>void;onSelect:(n:SimerTreeNode)=>void;onFollowLink:(n:SimerTreeNode)=>void;onOpenContainer:(n:SimerTreeNode)=>void}){
 const open=expanded.has(node.id);const children=node.children.length>0;const containerRef=/\\$?container[A-Za-z0-9_]+/i.test(node.nodeText);const containerRef=/\\$?container[A-Za-z0-9_]+/i.test(node.nodeText);
 return <Box id={`simer-node-${node.id}`} sx={{position:"relative",pl:level?2.4:0,scrollMargin:80}}>
  ...
  <Stack direction="row" spacing={.5} sx={{alignItems:"center",minHeight:34,position:"relative",zIndex:1}}>
   ...
   <Box onClick={()=>onSelect(node)} sx={{display:"flex",alignItems:"center",gap:.7,px:1,py:.45,border:"1px solid",borderColor:active===node.id?"primary.main":"divider",borderRadius:1.5,bgcolor:active===node.id?"action.selected":"background.paper",boxShadow:active===node.id?"0 0 0 2px rgba(24,199,122,.12)":"none",cursor:"pointer","&:hover":{borderColor:"primary.main",bgcolor:"action.hover"}}}>
    ...
    {containerRef&&<Tooltip title="Abrir definição do container"><IconButton size="small" color="primary" onClick={e=>{e.stopPropagation();onOpenContainer(node);}} sx={{width:22,height:22}}><LaunchOutlined sx={{fontSize:14}}/></IconButton></Tooltip>}{containerRef&&<Tooltip title="Abrir definição do container"><IconButton size="small" color="primary" onClick={e=>{e.stopPropagation();onOpenContainer(node);}} sx={{width:22,height:22}}><LaunchOutlined sx={{fontSize:14}}/></IconButton></Tooltip>}{node.link&&<Tooltip title="Abrir mapa vinculado"><IconButton size="small" color="primary" onClick={e=>{e.stopPropagation();onFollowLink(node);}} sx={{width:22,height:22}}><OpenInNewOutlined sx={{fontSize:14}}/></IconButton></Tooltip>}
   </Box>
  </Stack>
