...
import { AddOutlined, RemoveOutlined, AccountTreeOutlined, OpenInNewOutlined, UnfoldLessOutlined, UnfoldMoreOutlined, CenterFocusStrongOutlined } from "@mui/icons-material";
 ...
 return <Box id={`simer-node-${node.id}`} sx={{position:"relative",pl:level?2.4:0,scrollMargin:80}}>
  ...
  <Stack direction="row" spacing={.5} sx={{alignItems:"center",minHeight:34,position:"relative",zIndex:1}}>
   ...
   <Box onClick={()=>onSelect(node)} sx={{display:"flex",alignItems:"center",gap:.7,px:1,py:.45,border:"1px solid",borderColor:active===node.id?"primary.main":"divider",borderRadius:1.5,bgcolor:active===node.id?"action.selected":"background.paper",boxShadow:active===node.id?"0 0 0 2px rgba(24,199,122,.12)":"none",cursor:"pointer","&:hover":{borderColor:"primary.main",bgcolor:"action.hover"}}}>
    ...
    {node.link&&<Tooltip title="Abrir mapa vinculado"><IconButton size="small" color="primary" onClick={e=>{e.stopPropagation();onFollowLink(node);}} sx={{width:22,height:22}}><OpenInNewOutlined sx={{fontSize:14}}/></IconButton></Tooltip>}
   </Box>
  </Stack>
  {open&&children&&<Box sx={{ml:1.5,borderLeft:"1px solid",borderColor:"divider"}}>{node.children.map(child=><Branch key={child.id} node={child} level={level+1} expanded={expanded} active={active} onToggle={onToggle} onSelect={onSelect} onFollowLink={onFollowLink}/>)}</Box>}
 </Box>;
}
export function SimerMapTree({items,focusId,onSelect,onFollowLink}:{items:SimerTreeNode[];focusId?:number|null;onSelect:(n:SimerTreeNode)=>void;onFollowLink:(n:SimerTreeNode)=>void}){
 const roots=useMemo(()=>buildTree(items),[items]);const[expanded,setExpanded]=useState<Set<number>>(new Set());const[active,setActive]=useState<number|null>(focusId??null);
 useEffect(()=>{const next=new Set<number>();roots.forEach(r=>next.add(r.id));if(focusId){const target=items.find(x=>x.id===focusId);if(target){let parent=target.parentNodeId;next.add(target.id);while(parent){const p=items.find(x=>x.nodeId===parent);if(!p)break;next.add(p.id);parent=p.parentNodeId;}setActive(target.id);setTimeout(()=>document.getElementById(`simer-node-${target.id}`)?.scrollIntoView({behavior:"smooth",block:"center",inline:"center"}),80);}}setExpanded(next);},[items,focusId]);
 const center=()=>active&&document.getElementById(`simer-node-${active}`)?.scrollIntoView({behavior:"smooth",block:"center",inline:"center"});
 return <Box sx={{height:"68vh",minHeight:480,overflow:"auto",p:2,border:"1px solid",borderColor:"divider",borderRadius:3,bgcolor:"background.default"}}>
  <Stack direction="row" spacing={1} sx={{alignItems:"center",mb:1.5,position:"sticky",top:0,zIndex:3,bgcolor:"background.default",py:.5}}><AccountTreeOutlined color="primary"/><Typography sx={{fontWeight:900}}>Mapa navegável</Typography><Chip size="small" label={`${items.length.toLocaleString("pt-BR")} nós`}/><Box sx={{flex:1}}/><Button size="small" startIcon={<UnfoldMoreOutlined/>} onClick={()=>setExpanded(new Set(items.map(x=>x.id)))}>Expandir</Button><Button size="small" startIcon={<UnfoldLessOutlined/>} onClick={()=>setExpanded(new Set(roots.map(x=>x.id)))}>Recolher</Button><Button size="small" startIcon={<CenterFocusStrongOutlined/>} disabled={!active} onClick={center}>Centralizar</Button></Stack>
  <Box sx={{minWidth:"max-content",pb:3}}>{roots.map(root=><Branch key={root.id} node={root} expanded={expanded} active={active} onFollowLink={onFollowLink} onToggle={id=>setExpanded(v=>{const n=new Set(v);n.has(id)?n.delete(id):n.add(id);return n;})} onSelect={node=>{setActive(node.id);onSelect(node);}}/>)}</Box>
 </Box>;
}