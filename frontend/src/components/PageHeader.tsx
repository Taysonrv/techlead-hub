...
export function PageHeader({ eyebrow, title, description, meta, action }: { eyebrow: string; title: string; description: string; meta?: ReactNode; action?: ReactNode }) {
  ...
  return <Stack direction={{ xs: "column", lg: "row" }} sx={{ justifyContent: "space-between", alignItems: { xs: "stretch", lg: "center" }, gap: 2, mb: { xs: 2, md: 2.75 }, position: "relative", overflow: "hidden", p: { xs: 2, sm: 2.25, md: 2.6 }, borderRadius: { xs: 2.5, md: 3 }, isolation: "isolate", border: "1px solid", borderColor: dark ? "rgba(76,190,230,.20)" : "rgba(15,118,110,.13)", background: dark ? "radial-gradient(circle at 8% 0%, rgba(24,199,122,.13), transparent 34%), radial-gradient(circle at 92% 10%, rgba(71,108,255,.15), transparent 36%), linear-gradient(125deg, rgba(9,39,59,.98), rgba(13,31,57,.98) 58%, rgba(28,29,72,.96))" : "linear-gradient(115deg, #D8F8E9 0%, #DDF6EE 22%, #E1F1F4 55%, #E8EFFB 100%)", boxShadow: dark ? "inset 4px 0 0 #18C77A, 0 18px 42px rgba(0,0,0,.20), inset 0 1px rgba(255,255,255,.035)" : "inset 4px 0 0 #18C77A, 0 12px 30px rgba(16,24,40,.055)", "&::after": { display: "none" } }}>
