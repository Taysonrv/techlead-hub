...
export function ExecutiveSection({ title, subtitle, icon, action, children, accent = aliareColors.cyan, compact = false }: ExecutiveSectionProps) {
  ...
  return <Card elevation={0} sx={{ overflow: "hidden", borderColor: dark ? "rgba(90,174,220,.20)" : "divider", background: dark ? "linear-gradient(145deg, rgba(13,37,59,.96), rgba(8,24,41,.985))" : "linear-gradient(180deg,#FFFFFF,#FBFCFD)", "&::before": { content: '""', position: "absolute", inset: "0 0 auto", height: 2, background: `linear-gradient(90deg, ${accent}, color-mix(in srgb, ${accent} 35%, transparent), transparent 82%)`, boxShadow: dark ? `0 0 16px color-mix(in srgb, ${accent} 25%, transparent)` : "none" } }}>
