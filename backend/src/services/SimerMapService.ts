...
function kindFrom(icon:string|null,text:string){ const v=(icon??"").toLowerCase(),t=text.trim(); if(/^campo\s*:/i.test(t))return"campo"; if(/^(método|metodo|função|funcao)\s*:/i.test(t))return"metodo"; if(/^regra\s*:/i.test(t)||/^valid(ar|a|ação|acao|ar se)/i.test(t))return"regra"; if(/^container\s*:/i.test(t))return"container"; if(/^(serviço|servico)\s*:/i.test(t))return"servico"; if(/^classe\s*:/i.test(t))return"classe"; if(/^interface\s*:/i.test(t))return"interface"; if(v.includes("table"))return"objeto"; if(v.includes("package"))return"mapa"; if(v.includes("xmag"))return"seletor"; if(v.includes("filter"))return"filtro"; if(v.includes("cog"))return"servico"; if(v.includes("report"))return"relatorio"; if(v.includes("folder"))return"grupo"; if(/^(interface|objeto|classe|serviço|servico|consulta|relatorio|relatório|construtor|atributo|função|funcao|evento|seletor|utilitario|utilitário|campo|método|metodo|regra|container)$/i.test(t))return t.toLowerCase(); return null; }
