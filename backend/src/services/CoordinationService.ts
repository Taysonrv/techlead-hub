...
export class CoordinationService {
  ...
  async slaDevelopmentFlow(days = 180) {
    ...
    const tickets = await prisma.ticket.findMany({
      ...
      select: { movideskId:true, subject:true, client:true, owner:true, urgency:true, createdDate:true, taskNumber:true, solutionSlaIndicator:true }
    });
    ...
    const rows=tickets.flatMap(t=>{const w=t.taskNumber?byId.get(t.taskNumber):undefined;const p=urgency(t.urgency);if(!w||!w.azureCreatedAt||!p)return[];const concluded=norm(w.state)==="concluida";const end=concluded?(w.stateChangedAt??w.azureClosedAt??w.azureChangedAt):null;const support=businessMinutes(t.createdDate,w.azureCreatedAt);const factory=end?businessMinutes(w.azureCreatedAt,end):null;const total=end?businessMinutes(t.createdDate,end):null;const tg=targets[p];const supportPct=Math.round(support/tg.support*1000)/10;const factoryPct=factory===null?null:Math.round(factory/tg.factory*1000)/10;const totalPct=total===null?null:Math.round(total/tg.total*1000)/10;const bottleneck=factoryPct!==null&&factoryPct>supportPct?"Fábrica":"Suporte";return[{movideskId:t.movideskId,subject:t.subject,client:t.client,owner:t.owner??"Sem responsável",taskNumber:t.taskNumber,taskTitle:w.title,taskState:w.state,urgency:p,taskCreatedAt:w.azureCreatedAt,taskConcludedAt:end,supportMinutes:support,factoryMinutes:factory,totalMinutes:total,supportTargetMinutes:tg.support,factoryTargetMinutes:tg.factory,totalTargetMinutes:tg.total,supportPct,factoryPct,totalPct,bottleneck,officialSla:t.solutionSlaIndicator}]} );
    ...
    const monthly=[...new Set(rows.map(r=>monthKey(r.taskCreatedAt)))].sort().map(month=>{
      const group=rows.filter(r=>monthKey(r.taskCreatedAt)===month); const completed=group.filter(r=>r.taskConcludedAt);
      return {month,label:monthLabel(month),total:group.length,concluded:completed.length,avgSupportMinutes:avg(group.map(r=>r.supportMinutes)),avgFactoryMinutes:avg(completed.map(r=>r.factoryMinutes!)),avgTotalMinutes:avg(completed.map(r=>r.totalMinutes!)),supportWithinPct:group.length?Math.round(group.filter(r=>r.supportPct<=100).length/group.length*1000)/10:0,factoryWithinPct:completed.length?Math.round(completed.filter(r=>(r.factoryPct??Infinity)<=100).length/completed.length*1000)/10:0,totalWithinPct:completed.length?Math.round(completed.filter(r=>(r.totalPct??Infinity)<=100).length/completed.length*1000)/10:0,supportConsumptionPct:group.length?Math.round(group.reduce((a,r)=>a+r.supportPct,0)/group.length*10)/10:0,factoryConsumptionPct:completed.length?Math.round(completed.reduce((a,r)=>a+(r.factoryPct??0),0)/completed.length*10)/10:0};
    });
