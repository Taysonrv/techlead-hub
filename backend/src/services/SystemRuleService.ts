...
export class SystemRuleService{
 ...
 async importBpm(sourceFile:string,base64:string){sourceFile=normalizePath(sourceFile);const folderPath=folderOf(sourceFile);const outer=await JSZip.loadAsync(Buffer.from(base64,"base64"));let totalNodes=0,totalTransitions=0,processes=0,created=0,updated=0;
