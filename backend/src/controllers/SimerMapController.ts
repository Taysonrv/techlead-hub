...
export class SimerMapController {
  ...
  routine = async (req: AuthenticatedRequest, res: Response) => { const processId=Number(req.params.processId); const flow=await this.rules.flow(processId); if(!flow.process)return res.status(404).json({message:"Rotina não encontrada."}); const context=[flow.process.folderPath,flow.process.name,...flow.nodes.slice(0,12).map((n:any)=>n.name)].filter(Boolean).join(" "); const mapItems=await this.service.context(context,80); const [correlations,relatedProcesses]=await Promise.all([this.rules.correlate(context,mapItems,24),this.rules.relatedProcesses(processId,8)]); const maps=[...new Map(mapItems.map((m:any)=>[m.sourceFile,m])).values()].slice(0,12); const classify=(m:any)=>{const t=String(m.nodeText??"");if(/^campo\s*:/i.test(t))return"campo";if(/^(método|metodo|função|funcao)\s*:/i.test(t))return"metodo";if(/^regra\s*:/i.test(t)||/^valid(ar|a|ação|acao)/i.test(t))return"regra";if(/^container\s*:/i.test(t))return"container";if(/^(serviço|servico)\s*:/i.test(t))return"servico";if(/^classe\s*:/i.test(t))return"classe";return m.nodeKind??"elemento";}; const elements=correlations.map((m:any)=>({id:m.id,name:String(m.nodeText??"").replace(/^(campo|método|metodo|função|funcao|regra|container|serviço|servico|classe)\s*:\s*/i,""),kind:classify(m),mapName:m.mapName,path:m.path,score:m.correlationScore??m.score??0})); return res.json({process:flow.process,nodes:flow.nodes,transitions:flow.transitions,maps,correlations,elements,relatedProcesses,stats:{steps:flow.nodes.length,maps:maps.length,technicalPoints:mapItems.length,correlations:correlations.length}}); };
